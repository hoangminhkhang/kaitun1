...
wait(0.2)
	game.StarterGui:SetCore("SendNotification", {
		Title = "Kaitun Notification", 
		Text = "may bi cuttay a",
		Icon = "http://www.roblox.com/asset/?id=14334760790",
		Duration = 2
	})
	wait(0.1)
...
local PepsiUi = library:CreateWindow({
    Name = "Koben x Hub Kaitan Sereis x",
    Theme = {
					...
					if ac and ac.equipped then
						task.spawn(
							...
							if tick() - cdnormal > 9.0 then
								...
								game:GetService("ReplicatedStorage").RigControllerEvent:FireServer("hit", getHits(150), 2, "")
							end
						end)
					end
				end
